...
bool accept_websocket_si_demande(char *req, char *rep, int debut_header) {
    ...
    bool res = (regexec(&regex_upgrade_websocket, rep, 0, NULL, 0) == 0) &&
               (regexec(&regex_connection_upgrade, rep, 0, NULL, 0) == 0) &&
               (regexec(&regex_websocket_key, rep, 4, match_key, 0) == 0);
    if (! res) return FALSE;
    strncpy(cle_ascii, rep + match_key[2].rm_eo, 16);
